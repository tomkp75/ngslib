#!/usr/bin/python
#Last-modified: 16 Mar 2015 11:26:57 PM
...
def fetchchrom(sam,chrom,binsize=50,extend=150,normedto=10000000,paried=False,forcestrand=False):
    ''' Fetch reads in one chrom. '''
    if forcestrand:
        bins={"+":{},"-":{}}
    else:
        bins={".":{}}
    chrlen = sam.lengths[sam.references.index(chrom)]
    for strand in bins:
        bins[strand] = numpy.zeros(chrlen/binsize+extend/binsize+2,'float')
    ...
    if not paired:
        start=stop=0
        strand="."
        for read in sam.fetch(chrom): # sam is 0-based
            if not read.is_unmapped:
                ...
                bins[strand][startbin]-=start%binsize 
                # For stopbin:
                ...
                bins[strand][stopbin] +=stop%binsize+1 
                bins[strand][startbin:stopbin]+=binsize
...
def openWigFile(prefix,outputdir,samfile,bins,control,ctlbins=None,chrsplit=False):

#!/usr/bin/python
#Last-modified: 24 Mar 2015 01:57:22 PM
...
def argParser():
    ...
    p.add_argument("-g",dest='genome',type=str,metavar='genome',required=True,help="Chrom sizes file.")
    ...
    p.add_argument("-n",dest="normedto",type=int,metavar="normedto",default=10,help="Reads normalization . Set \"0\" if no normalization. Set \"10\" if want to normalized to 10 M reads. Note: if overlapped Beds are merged, multiply n by the length of reads. For example, if read length is 36bp and we want to normalized to 10M reads, set n = 36*10 = 360. [default= 10 M]")
    p.add_argument("--byscore",dest="byscore",action='store_false',help="Set if Beds are merged from reads. By default, Beds are from original reads.")
    p.add_argument("-R","--RNASeq",dest="RNASeq",action='store_true',help="Input is RNA Seq reads. No extension by default.")
    p.add_argument("-P","--Paired",dest="Pairend",action='store_false',help="Input is paired end reads. No extension by default.")
    p.add_argument("-s","--split",dest="split",action='store_false',help="Split wiggle into separate files by chromosomes.")
    p.add_argument("-f","--forcestrand",dest="forcestrand",action='store_false',help="Force strand.")
...
#        print >>sys.stderr, "Prcessed ",cnt, "reads.     "
    ...
    # Normalization
    ratio= normedto==0 and 1.0 or float(normedto)/rcnt
